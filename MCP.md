...
mailX 프로젝트에서 사용하는 MCP(Model Context Protocol) 서버의 통합 가이드입니다.
...
```
find_symbol("MailXDbContext")
→ Data/MailXDbContext.cs:8 정확한 위치 반환
```
...
```
get_symbols_overview("mailX/Data/MailXDbContext.cs")
...
```
get_symbols_overview("mailX/Services/Graph/GraphMailService.cs")
...
### 사용 시 주의사항
- mailX는 SQLite를 사용하므로 MySQL MCP는 주로 외부 DB 연동 시 사용
- UTF-8 인코딩 주의 (한글 데이터)
