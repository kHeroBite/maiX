...
## L-545: 서버VAD STT 카드 인터리브 — 단일base 문자열 모델은 근본 한계, item별 순서보존 누적이 정답 (2026-07-21)

- **문제**: 서버VAD 실시간 STT 카드에서 텍스트 진동/중복/순서뒤섞임이 3회 수정에도 잔존. 082f63e4(카드 base+누적 재구성)/89e2998c(completed 핸들러 누적 보존)가 각각 sameItem 덮어쓰기 버그와 completed 병합 버그를 고쳤으나 근본 증상은 계속 재발.
- **근본원인 (nlog `[카드누적]` 실측으로 확정)**: 서버VAD 모드에서는 **한 카드(cardKey)에 다중 OpenAI item이 인터리브(delta가 도착 순서대로 뒤섞여 옴)** 되는데, 기존 `_cardBaseTexts`는 "카드당 단일 문자열(이전 병합분 스냅샷)" 모델이라 이 인터리브를 구조적으로 표현할 수 없었음. base+accum 이어붙이기로는 어떤 순서 조합을 짜도 특정 인터리브 패턴에서 진동/중복이 재발.
- **해결 (데이터모델 교체)**: `_cardBaseTexts`(단일 문자열) 폐기 → `_cardItemOrder`(cardKey별 itemId 등장순 List) + `_cardItemFinalTexts`(itemId별 완료 확정 텍스트) 신설. `RebuildCardText(cardKey)`가 등장순으로 각 item의 텍스트(완료면 `_cardItemFinalTexts`, 진행중이면 `_deltaBuffers`)를 join하여 카드 전체 텍스트를 그때그때 재조립. delta/completed 양쪽 핸들러 모두 이 재조립 함수로 통일.
- **교훈**: 카드/그룹 단위 텍스트 누적 버그가 단발 수정 2회+에도 재발하면, "누적 로직 자체를 고치는 것"이 아니라 "데이터모델이 표현 불가능한 상태를 강요하고 있는지"를 의심해야 함. 이번 케이스는 카드:item = 1:N(인터리브) 관계인데 모델이 1:1(단일 문자열)을 가정 — 관계 카디널리티 오분석이 근본원인.
- **진단 결정타**: nlog `[카드누적]` 실측 로그로 delta가 여러 itemId에서 도착순으로 인터리브되는 것을 직접 확인한 것이 결정적. 추측 수정 반복 대신 로그 실측 우선 원칙 재확인.
- **심각도**: 중간 (반복 역라우팅 3회 — 매몰비용 신호였음)
- **Level**: 2 (MEMORY.md 기록 — 카드/그룹 누적 버그 재발 시 데이터모델 카디널리티부터 의심)
- **수정 파일**: OpenAiRealtimeSttService.cs (`_cardItemOrder`+`_cardItemFinalTexts`+`RebuildCardText` 신설, `_cardBaseTexts` 전면 제거)
- **연관**: L-446(외부 API 디버깅 로그 실측 우선), L-528(단발 추측 수정 2회+ 매몰비용), L-530(증상≠원인 메타교훈)
- **커밋**: 082f63e4(1차 base+누적) → 89e2998c(2차 completed 보존) → 본 수정(3차 근본 데이터모델 교체)
- **런타임검증**: 자동검증(빌드+동시성) PASS. 런타임 발화검증(진행중진동/최종순서/reconnect/auto-split)은 사용자 재기동 테스트 대기 (신규빌드 nlog itemCount= 포맷 로그 미확보)
- **대화ID**: conv_178462254443
...
## L-530: TimeSpan mm 포맷 0~59 wrap-around — 60분 이상 경과시간은 TotalMinutes 사용 필수 (2026-06-18)

- **문제**: C# TimeSpan 커스텀 포맷 `mm`은 분을 0~59로만 표시. 60분 경과 시 "01:00:00" 대신 "00:00"으로 리셋.
- **근본원인**: `ToString(@"mm\:ss")` 패턴은 TimeSpan.Minutes(0~59 범위)를 사용. 60분 경과 시 Minutes=0 → wrap.
- **해결**: `TimeSpanFormatter.FormatTimeSpan()` 헬퍼 도입 → `(int)ts.TotalMinutes`로 단조 증가 보장. 60분 미만: `mm:ss`, 60분 이상: `h:mm:ss`.
- **교훈**: **TimeSpan 커스텀 포맷 `mm` 패턴은 1시간 미만 보장 시에만 사용.** 녹음/재생 경과시간 등 무한 증가 가능한 값에는 `(int)ts.TotalMinutes` 필수. 코드 리뷰 시 `` ToString(@"mm `` / `:mm\:ss` grep으로 전수 점검.
- **재발방지**: 경과시간 포맷 코드 리뷰 시 `ToString(@"mm` 패턴 존재하면 1시간 제한 보장 여부 확인.
- **심각도**: 중간 (사용자 가시 버그 — 60분 경계에서 시간 표시 리셋)
- **Level**: 1
- **연관**: 없음
- **대화ID**: conv_odone_20260618

---

## L-531: 표시 포맷 버그 vs 처리 동작 — otest 검증 범위 명시 필수 (2026-06-18)

- **문제**: 사용자 신고는 "STT/녹음/요약/MAP 처리 멈춤"이었으나 근본 원인은 표시 포맷 wrap이었음. 표시 버그가 처리 중단처럼 보일 수 있음.
- **근본원인**: 60분 경계에서 타이머가 "00:00"으로 리셋되면 사용자는 새 녹음이 시작된 것처럼 인식. 실제 처리 파이프라인(STT/요약)은 계속 동작 중이었을 가능성 있음.
- **해결**: oplan이 STT 세션 재시작 로직 부재를 코드로 확인했고 otest가 포맷 단조증가 실측했으나, 60분 이상 실제 런타임 전수 검증은 미수행.
- **교훈**: **표시 버그와 처리 버그는 구분해서 검증해야 한다.** otest acceptance_criteria에 "표시 레이어 검증 완료, 처리 레이어는 사용자 실사용 확인 필요"를 명시. otest는 표시 포맷까지만 검증 가능.
- **재발방지**: otest 완료 보고 시 "자동 검증 범위: ~, 사용자 확인 필요 범위: ~"를 명시 (L-529 연장).
- **심각도**: 낮음 (정직성/검증 한계 명시)
- **Level**: 1
- **연관**: L-529 (헤드리스 런타임 검증 한계), L-446 (외부 API 디버깅 장기전)
- **대화ID**: conv_odone_20260618
...
## L-545: STT 진단 로그 3채널 분산 — 채널 전수 확인 없이 0건=미동작 단정 금지 (2026-07-13)

- **문제**: 원노트탭 실시간 STT 2계층 VAD 카드 병합 작업 중, 메인이 [STT진단]/[MainWindow] 로그를 nlog-*.log/mAIx-*.log에서 찾다가 0건이라 "화면 갱신 경로 끊김"으로 오진.
- **근본원인**: STT 관련 진단 로그가 3개 채널에 분산됨 — nlog-*.log(STT 이벤트), mAIx-*.log, 20260713.log(UI/진단). 실제로는 20260713.log에 108건/114건 존재했음.
- **해결**: 채널 전수 확인(nlog-*.log + mAIx-*.log + YYYYMMDD.log) 후 판단.
- **교훈**: L-406/L-408(로그 채널 혼동) 재확인 사례. "0건=미동작" 단정 전 3채널 전수 확인 필수.
- **심각도**: 중간 (오진 → 불필요한 역라우팅 위험, 단 이번엔 조기 발견으로 회피)
- **Level**: 1 (기존 L-406/L-408 원칙 재확인 — 신규 규칙 불필요)
- **연관**: L-406, L-408
- **대화ID**: conv_178390796062

## L-546: 필드 선언만 있고 로직 미구현 상태를 "구현됨"으로 오판하기 쉬움 — 호출부 grep 필수 (2026-07-13)

- **문제**: "옵션 C: N초 침묵 기반 카드 분할" 주석과 `_currentSpeechItemId`/`_lastDeltaAt`/`SilenceCardThresholdSec` 필드 선언이 있었으나, 실제 침묵 갭 비교 로직은 전혀 없는 미완성 상태였음.
- **근본원인**: 필드 선언 + 의미있는 주석의 존재가 "구현 완료"로 오인되기 쉬움. 정의부만 확인하고 호출부(실제 사용처)를 확인하지 않으면 미구현 상태를 놓침.
- **해결**: oplan 단계에서 정의부+호출부 동시 grep으로 실제 로직 부재를 사전 발견 → "수정"이 아닌 "신규 구현"으로 재분류.
- **교훈**: L-419(정의부+호출부 동시 확인) 재확인 사례. 주석 신뢰보다 호출부 grep 우선.
- **심각도**: 중간 (구현 범위 오판 → 계획 재수립 필요했을 뻔함, 이번엔 oplan에서 조기 발견)
- **Level**: 1 (기존 L-419 원칙 재확인 — 신규 규칙 불필요)
- **연관**: L-419
- **대화ID**: conv_178390796062

## L-547: gpt-4o-transcribe VAD 세그먼트 종료 후에만 delta batch 방출 — 서버 VAD 튜닝+클라이언트 병합으로 우회 (2026-07-13)

- **문제**: gpt-4o-transcribe 모델이 발화 도중 partial(진행 중) delta를 제공하지 않고, 서버 VAD 세그먼트가 종료된 후에만 delta를 batch로 방출하여 발화 중 화면이 비는 문제.
- **근본원인**: OpenAI Realtime API의 gpt-4o-transcribe 계열은 서버측 구조상 세그먼트 완료 전 partial 미제공(모델 capability 제약, 클라이언트 우회 불가). gpt-realtime-whisper는 진짜 partial을 주나 전사 품질이 나쁨.
- **해결**: 모델 교체 대신 서버 VAD(VadSilenceDurationMs)를 짧게 하여 delta를 자주 받고, 클라이언트에서 긴 침묵 갭(CardMergeSilenceThresholdSec, 신설) 기준으로 하나의 화자 카드로 병합하는 2계층 방식 채택.
- **교훈**: L-447 계열 재확인. 모델 capability 제약이 근본 원인일 때는 모델 교체보다 서버/클라이언트 파라미터 분리 튜닝을 우선 검토.
- **심각도**: 낮음 (설계 검증 사례 — 정상 진행)
- **Level**: 1
- **연관**: L-447
- **대화ID**: conv_178390796062

## L-548: 마이크 녹음 등 UI 인터랙션 필요 런타임 검증은 정적 PASS + 사용자 RETEST로 분리 (2026-07-13)

- **문제**: 2계층 VAD 카드 병합 로직([병합판정] 로그)의 실제 발화 검증에 실제 마이크 음성 입력 + 로그인 후 녹음 시작이라는 UI 인터랙션이 필요했으나, otest는 마이크 입력을 자동 발생시킬 수단이 없음(Playwright/FlaUI로 마이크 오디오 스트림 자동 발화 불가).
- **해결**: 빌드 PASS + 병합 판정 로직 코드 구현 확인(정적 검증)까지 완료 후, 런타임 로그 실측은 `evidence/otest_deferred.md`로 이연하여 사용자 재녹음 후 RETEST로 분리. otest는 PARTIAL_PASS로 odone 진입.
- **교훈**: L-529(UI 인터랙션 필요 시나리오 자동/수동 검증 분리) 재확인 사례.
- **심각도**: 낮음 (검증 범위 분리 — 정상 처리)
- **Level**: 1
- **연관**: L-529
- **대화ID**: conv_178390796062
...
## L-553: 대립 증상(소실 vs 과잉분리)은 동일 근본(상태맵 생명주기) 양면일 수 있음 — 한쪽만 막으면 진자가 반대로 넘어간다 (2026-07-22)

- **문제**: 서버VAD 실시간 STT 카드 버그를 5회에 걸쳐 수정. 4회(082f63e4/89e2998c/bf1998ee/b55a143b) 모두 증상 표면(소실/축소)에 대응한 오진이었음.
- **근본원인**: 진짜 근본은 "카드 재사용 자체"가 아니라 "재사용되는 카드의 상태맵(`_cardItemOrder`/`finalTexts`)이 completed 정리에서 파괴되어 `RebuildCardText`가 빈 문자열을 반환"하는 것이었음. b55a143b가 "재사용 금지(`_currentSpeechItemId=null`)"로 오처리 → 병합 2254건→8건으로 전멸(진자 반전).
- **해결**: 재사용은 허용하되 병합대상 카드의 상태맵을 보존하고, 새 카드 생성 시에만 old를 evict하는 방식으로 양립 해결.
- **교훈**: (1) 트레이드오프 대립처럼 보이는 두 증상(소실 vs 과잉분리)이 실은 동일 근본(상태맵 생명주기)의 양면일 수 있다 — 한쪽만 막으면 진자가 반대로 넘어간다. (2) 증상 표면 대응 4회 실패 = 데이터 생명주기(상태맵 소유권/파괴 시점)를 정면으로 봐야 한다. (3) nlog 정량 실측(병합 2254→8건 비교)이 진자 반전을 객관 확정한 결정타.
- **심각도**: 높음 (5회 반복 역라우팅 — 핵심 UX 기능 장기 미해결)
- **Level**: 2 (신규 규칙 — 대립 증상 재발 시 공통 근본 상태맵/생명주기 우선 의심)
- **연관**: L-446, L-528, L-530
- **수정 파일**: `mAIx/Services/AI/OpenAiRealtimeSttService.cs` (카드 재사용 허용 + 상태맵 보존 + 새카드 evict 3단계)
- **대화ID**: conv_178468117703
- **대화ID**: conv_178451224854
