...
음성_파이프라인_2모드_시스템 (신규 — 2026-05-15, L-439~L-443):
  - 파일명: AudioPipelineMode.cs (신규)
    경로: Services/AI/
    역할: enum — Legacy / Unified
    설명: 음성 파이프라인 모드 식별자

  - 파일명: IRealtimeAudioPipeline.cs (신규)
    경로: Services/AI/
    역할: 음성 파이프라인 공통 인터페이스
    이벤트: TranscriptSegmentReceived, MinuteSummaryCreated, PipelineFallback, CostUpdated
    메서드: StartAsync(), StopAsync(), DisposeAsync(), SubscribeEvents(), UnsubscribeEvents()
    연관: L-440 (인터페이스+팩토리 도입)

  - 파일명: AudioPipelineFactory.cs (신규)
    경로: Services/AI/
    역할: 모드 enum → 구현체 인스턴스 생성 + 자동 폴백 인스턴스 생성
    메서드: Create(AudioPipelineMode), CreateLegacyFallback(...)
    연관: L-440

  - 파일명: LegacyAudioPipeline.cs (신규, ~150줄)
    경로: Services/AI/
    역할: 기존 STT(OpenAiRealtimeSttService/OpenAiTranscribeSttService) + 별도 1분 요약 + 별도 감성 분석 호출
    인터페이스: IRealtimeAudioPipeline 구현
    연관: SentimentAnalysisService (별도 감성 호출)

  - 파일명: UnifiedRealtimeAudioPipeline.cs (신규, ~827줄)
    경로: Services/AI/
    역할: gpt-realtime/-2/-mini 단일 WebSocket — STT + 1분 요약 + 감성 분석 통합
    인터페이스: IRealtimeAudioPipeline 구현
    핵심_패턴: out-of-band response.create (create_response=false + tools strict function_call + item_reference 슬라이딩 윈도우 N=8)
    동시성: _sendLock SemaphoreSlim(1,1) (L-443)
    연관: L-441 (out-of-band), L-442 (5단계 swap), L-443 (_sendLock)

  - 파일명: SentimentAnalysisService.cs (신규)
    경로: Services/AI/
    역할: gpt-4o-mini 기반 감성 분석 (Legacy 모드 전용)
    반환: SentimentResult (Score 0~100 + Emoji + Label)

  - 파일명: CostEstimatorService.cs (신규)
    경로: Services/AI/
    역할: 모델별 분당 예상 비용 계산 (입력/출력 토큰 가중치 기반)
    사용처: OneNoteViewModel.UpdateCostDisplay

  - 파일명: Helpers/HallucinationFilter.cs (신규)
    경로: Services/AI/Helpers/
    역할: STT 환각 텍스트 필터링 헬퍼 (정규식 + 휴리스틱)
    사용처: UnifiedRealtimeAudioPipeline, LegacyAudioPipeline

  - 파일명: SentimentScoreToColorConverter.cs (신규)
    경로: Converters/
    역할: 감성 Score(0~100) → 그라데이션 색상 IValueConverter
    바인딩: MainWindow.xaml DataTemplate (1분 요약 카드 배경)

  - 모델_변경:
    - MinuteSummaryEntry.cs: +Sentiment 필드 (SentimentResult), +CreatedByMode 필드
    - RealtimeRecordingResult.cs: +RecordedWithMode 필드
    - Settings/OpenAiRecordingSettings.cs: +4 신규 필드 (모드 선택, Unified 모델, 폴백 임계값, 자동 폴백 enable)

  ViewModel_통합 (OneNoteViewModel.cs):
    - 신규 필드: `_audioPipeline` (IRealtimeAudioPipeline)
    - 신규 메서드: OnPipelineFallback (5단계 대칭 swap 핸들러), UpdateCostDisplay, 3 partial 메서드
    - 모드 선택 시 즉시 swap (Settings 변경 직후)

  XAML_통합 (MainWindow.xaml):
    - Settings 패널: 3컨트롤 (모드 선택 ComboBox, Unified 모델 ComboBox, 자동 폴백 토글)
    - 1분 요약 DataTemplate: 감성 결과(이모지 + 라벨 + 그라데이션 배경) 표시
    - 비용 미리보기: 모드별 분당 예상 비용 텍스트

  DI_등록 (App.xaml.cs):
    - SentimentAnalysisService, CostEstimatorService, AudioPipelineFactory 등록
    - HallucinationFilter Singleton 등록
    - SentimentScoreToColorConverter App.xaml 리소스 등록
```
