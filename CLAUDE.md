...
| 스킬 | 설명 |
|------|------|
| **kO** | 디스패처 + 직접 오케스트레이터 — 모든 메시지에 자동 발동, 3-way 분류(질문/퀵/코드수정). 코드 수정 시 kO가 직접 kPlan→kDev→kTest→kDone 팀에이전트 4단계 오케스트레이션. 추가 작업 시 kO-worker 위임 |
...
> **파이프라인 순서 엄수 (L-008)**: kO 직접 오케스트레이션으로 kPlan → 분류 → kDev → kTest → kDone. 이전 단계 완료 전 다음 진입 금지.
