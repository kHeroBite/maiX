...
**모든 사용자 메시지에 kO 자동 발동** → 4-way 분류 후 on-demand 서브스킬 로드 (토큰 절약)

```
사용자 메시지 → [kO] 오케스트레이션 (모든 메시지에 자동 발동)
                 ├─ 질문/탐색 → 직접 응답 (스킬 미사용)
                 ├─ 라이트 작업 → kDev(메인 직접) → kTest → kDone(Fast)
                 ├─ 미디엄 작업 → kPlan(Quick) → kDev(에이전트 1개+) → kTest → kDone(Fast)
                 └─ 풀 작업 → kO_gate → kPlan → kDev(에이전트 2개+) → kTest → kDone
```
...
| 스킬 | 설명 |
|------|------|
| **kO** | 오케스트레이션 진입점 — 모든 메시지에 자동 발동, 4-way 분류 |
| **kO_gate** | 병렬화 판정 — 규모 산정, 파일 할당 매트릭스, 에이전트 수 결정 |
...
환경변수_설정:
  ...
  CLAUDE_AUTOCOMPACT_PCT_OVERRIDE: "80"         # 80% 도달 시 자동 압축
...
Context_관리: 30% 이하 시 자동 /compact 실행
```
