...
## 2026-05-22 — 마인드맵 radial 사이클: d3 v7 방사형 + 디스크 영속화 + 우클릭 편집 + PropertyChanged 재등록

| 2026-05-22 | 🔧 마인드맵 radial 사이클 — d3 v7 방사형 360° + 디스크 영속화 + 우클릭 메뉴 + PropertyChanged 재등록. ① markmap 3개 제거 + d3.v7.min.js 신규 임베드(279,706 bytes). ② MindMapTreeFile.cs 신규 모델(Markdown/IsUserEdited/UpdatedAt). ③ MindMapTreeService: SetCurrentRecording/LoadFromDiskAsync/SaveToDiskAsync/SaveUserEditedAsync — 원자적 .tmp→Move 교체 + FileShare.ReadWrite. ④ mindmap.html 전면 재작성: d3 radial tree([2π,radius])+d3.linkRadial, parseMarkdown/serializeToMarkdown/renderTree, contextmenu 우클릭 div 메뉴, {type:'close'}/{type:'tree_edited'} JSON postMessage. ⑤ MindMapOverlay.xaml.cs: Bind 시 clearMindMap+Reset+SetCurrentRecording+LoadFromDisk, OnWebMessageReceived JSON 분기(close/tree_edited), HandleTreeEditedAsync+SaveUserEditedAsync. ⑥ MainWindow.OneNote.cs: PropertyChanged 재등록(토글 ON 시 -= += 재등록), IsMindMapVisible 조건(IsVisible 렌더 타이밍 함정 회피). 정적 32건+회귀 4건+빌드 PASS. ⚠️ 사용자 UI 실측 15건 보류(d3 SVG/contextmenu/편집저장/녹음전환 등). 신규 교훈: L-500~L-511. | mAIx/Services/AI/MindMapTreeService.cs (수정), mAIx/Models/MindMapTreeFile.cs (신규), mAIx/Resources/mindmap.html (전면 재작성), mAIx/Resources/d3.v7.min.js (신규), mAIx/Resources/markmap-d3.js (삭제), mAIx/Resources/markmap-lib.js (삭제), mAIx/Resources/markmap-view.js (삭제), mAIx/mAIx.csproj (수정), mAIx/Controls/MindMapOverlay.xaml.cs (수정), mAIx/Views/MainWindow.OneNote.cs (수정) |
...
## 2026-05-21 — 파이프라인 스킬 ogrill 선행 호출 게이트 일관 추가 (17 파일)

| 2026-05-21 | ✨ ogrill 선행 호출 게이트 메타 작업 — AC-G01~G14 전수 PASS. ogrill/SKILL.md에 선행 호출 매트릭스 신규 섹션 추가(단일 출처 DRY); CLAUDE.md에 ogrill 선행 호출 게이트 절대 규칙 추가; ok/o1~o5/oplan 계열/odebate/oconsult/odebug 등 15개 스킬에 ogrill 선행 호출 게이트 섹션 일관 삽입. 신규 교훈: L-485(단일 출처+일관 참조 패턴), L-486(LLM 자가 판단 게이트 패턴). | .claude/skills/ogrill/SKILL.md, CLAUDE.md, ok/SKILL.md, o1~o5/SKILL.md, oplan/SKILL.md, oplan_simple/oplan_normal/oplan_deep/oplan_debate/oplan_consult/SKILL.md, odebate/oconsult/odebug/SKILL.md (17파일) |
